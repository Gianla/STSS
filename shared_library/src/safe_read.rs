...
pub fn safe_read(
    ...
    if let Some(e_extension) = expected_extension {
        ...
        if filepath
            .extension()
            .is_none_or(|ext| ext != e_extension.as_ref())
        {
    ...
    let filesize = usize::try_from(file_len).map_err(|_| {
        SafeReadError::Memory(
            filepath.clone(),
            "file is too large (around 4 GB or more)".to_string(),
        )
    })?;
    ...
    buffer
        .try_reserve(filesize)
        .map_err(|e| SafeReadError::Memory(filepath.clone(), e.to_string()))?;
    ...
    safe_reader
        ...
        .map_err(|e| SafeReadError::Read(filepath.clone(), e.to_string()))?;
