...
use crate::server::{KeyContext, RuntimeContext, ServerContext};
...
impl Config {
    pub fn new(network: NetworkConfig, runtime: RuntimeConfig, keys: KeysConfig) -> Self {
        Self {network, runtime, keys}
    }
}
...
pub enum ServerConfigConversionError {
    ...
    #[error("port has been set to zero: although this means to let the OS decide it, the Client \
...
impl Config {
    ...
    pub fn from_file(path: impl Into<PathBuf>) -> Result<Self, ConfigError> {
        let raw_toml = String::from_utf8(safe_read(path, TOML_EXT)?).map_err(|e| {
            ConfigError::TomlNotInUtf8(e.to_string())
        })?;

        let conf: Config = toml::from_str(&raw_toml).map_err(|e| {
    ...
    pub fn get_ca_certificate(self) 
        ...
        let certs = rustls_pemfile::certs(&mut ca_cert_bytes_slice)
            ...
            .map(|cert| cert.map_err(|e|
                ServerConfigConversionError::CertificateParse(e.to_string())
            ))
    ...
    pub fn to_servercontext(self) -> Result<ServerContext, ServerConfigConversionError> {
        ...
        let tls_priv = rustls_pemfile::private_key(&mut tls_key_bytes_slice)
            ...
            .ok_or_else(|| ServerConfigConversionError::TlsKeyNotFound)?;
        ...
        Ok(
            ServerContext::new(
                ...
                RuntimeContext::new(self.runtime.working_threads, self.runtime.cryptography_threads),
